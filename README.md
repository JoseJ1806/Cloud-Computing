...
El código de la aplicación sencilla a probar no se pudo implementar en dos de los PaaS seleccionados (Microsoft Azure y Heroku), y en el tercero seleccionado (Google Cloud Platform)mostraba la aplicación en la web pero a la hora de correr el código por ultima vez tiro un error que no se pudo solucionar a tiempo (este error se puede ver en el repositorio del codigo y pasa a la hora de implementar la linea "gcloud app deploy" en el shell). Por ende no se pueden comparar en el aspecto de experiencias. En precio las 3 eran de uso gratuito pero ofrecian una versión premium con asistencia 24/7 y en la implementación Microsoft Azure fue la mejor de todas, los menus eran faciles de entender y la plataforma como tal era fácil de entender, algo en lo que sufrio mucho Google Cloud Platform ya que no es intuitiva, aun viendo la documentación este aspecto no mejoraba.
